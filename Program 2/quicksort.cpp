...
void QuickSort(std::vector<int>* numbers, int& numMemAcc, int& numComp) {
   QuickSortRecurse(numbers, 0, numbers->size() - 1,numMemAcc, numComp);
}
...
void QuickSortRecurse(std::vector<int>* numbers, int i, int k, int& numMemAcc, int& numComp) {
   ...
   j = Partition(numbers, i, k, numMemAcc, numComp);
   
   /* Recursively sort low partition (i to j) and
    high partition (j + 1 to k) */
   QuickSortRecurse(numbers, i, j, numMemAcc, numComp);
   QuickSortRecurse(numbers, j + 1, k, numMemAcc, numComp);
